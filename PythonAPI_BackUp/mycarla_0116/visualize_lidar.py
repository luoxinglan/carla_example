...
class SensorManager:
    ...
    def save_lidar_image(self, image):
        ...
        response = requests.post("http://localhost:8001/upload-image/lidar/5",
                                 files={"file": ("lidar_frame.jpg", img_str)})
    ...
    def save_semantic_image(self, image):
        ...
        # 将语义分割图像发送到 FastAPI 服务器
        response = requests.post("http://localhost:8001/upload-image/semantic/5",
                                 files={"file": ("semantic_frame.jpg", img_str)})
        print(f"语义分割图像响应来自服务器: {response.json()}")
...
def main():
    ...
    # 添加 LiDAR 传感器
    lidar_sensor_manager = SensorManager(world, 'LiDAR', carla.Transform(carla.Location(x=0, z=2.4)), vehicle, {
        'channels': '64',
        'range': '100',
    ...
    # 添加语义分割相机
    semantic_sensor_manager = SensorManager(world, 'SemanticSegmentationCamera',
    ...
    try:
        ...
        lidar_sensor_manager.destroy()
        semantic_sensor_manager.destroy()
