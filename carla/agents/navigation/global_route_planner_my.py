...
class GlobalRoutePlanner(object):
    ...
    def __init__(self, wmap, sampling_resolution):
        ...
        :param wmap: World map object.
        :param sampling_resolution: Sampling resolution.步长之类的东西
        """
    ...
    def trace_route(self, origin, destination):
        """
        该方法返回从起点到终点的(carla.Waypoint, RoadOption)列表

        参数:
        - origin: 起点的位置坐标（CARLA世界坐标系）
        - destination: 终点的位置坐标（CARLA世界坐标系）

        返回:
        - 包含(carla.Waypoint, RoadOption)元组的列表，其中RoadOption指示跟随的方向。
        """
        route_trace = []  # 初始化一个空列表来存储追踪的路径
        route = self._path_search(origin, destination)  # 查找从起点到终点的路径
        current_waypoint = self._wmap.get_waypoint(origin)  # 获取起点对应的waypoint
        destination_waypoint = self._wmap.get_waypoint(destination)  # 获取终点对应的waypoint

        for i in range(len(route) - 1):  # 遍历路径中的waypoints，除了最后一个
            road_option = self._turn_decision(i, route)  # 确定当前waypoint的道路选项
            edge = self._graph.edges[route[i], route[i + 1]]  # 获取当前waypoint和下一个waypoint之间的图边
            path = []

            if edge['type'] != RoadOption.LANEFOLLOW and edge['type'] != RoadOption.VOID:
                # 如果边类型不是LANEFOLLOW或VOID，将当前waypoint和道路选项添加到路由跟踪中
                route_trace.append((current_waypoint, road_option))
                exit_wp = edge['exit_waypoint']  # 获取当前边的出口waypoint
                n1, n2 = self._road_id_to_edge[exit_wp.road_id][exit_wp.section_id][
                    exit_wp.lane_id]  # 获取由出口waypoint连接的节点ID
                next_edge = self._graph.edges[n1, n2]  # 根据节点ID获取下一条边
                if next_edge['path']:
                    # 如果下一条边有路径，找到路径中最接近当前waypoint的waypoint
                    closest_index = self._find_closest_in_list(current_waypoint, next_edge['path'])
                    closest_index = min(len(next_edge['path']) - 1, closest_index + 5)  # 调整索引以确保其在范围内
                    current_waypoint = next_edge['path'][closest_index]  # 将当前waypoint更新为找到的最接近的一个
                else:
                    current_waypoint = next_edge['exit_waypoint']  # 如果没有路径存在，将当前waypoint更新为出口waypoint
                route_trace.append((current_waypoint, road_option))  # 将更新后的waypoint和道路选项添加到路由跟踪中

            else:
                # 如果边类型是LANEFOLLOW或VOID，使用入口、路径和出口waypoint构造路径
                path = path + [edge['entry_waypoint']] + edge['path'] + [edge['exit_waypoint']]
                closest_index = self._find_closest_in_list(current_waypoint, path)  # 在构造的路径中找到最接近的waypoint
                for waypoint in path[closest_index:]:  # 遍历路径中剩余的waypoints
                    current_waypoint = waypoint  # 更新当前waypoint
                    route_trace.append((current_waypoint, road_option))  # 将当前waypoint和道路选项添加到路由跟踪中
                    if len(route) - i <= 2 and waypoint.transform.location.distance(
                            destination) < 2 * self._sampling_resolution:
                        break  # 如果剩余路径长度小于等于2且waypoint距离目的地小于两倍采样分辨率，则跳出循环
                    elif len(
                        ...
                        # 如果剩余路径长度小于等于2且当前waypoint与目的地waypoint在同一道路上，则检查是否需要提前结束
                        destination_index = self._find_closest_in_list(destination_waypoint, path)
                        if closest_index > destination_index:
                            break  # 如果当前索引大于目标索引，则跳出循环
